...
**Note:** this is the "per-architecture" repository for the `riscv64` builds of [the `adminer` official image](https://hub.docker.com/_/adminer) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![riscv64/adminer build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/riscv64/job/adminer.svg?label=riscv64/adminer%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/riscv64/job/adminer/)
...
```console
$ docker run --link some_database:db -p 8080:8080 riscv64/adminer
```
...
```console
$ docker run --link some_database:db -p 9000:9000 riscv64/adminer:fastcgi
```
...
```console
$ docker run --link some_database:db -p 8080:8080 -e ADMINER_PLUGINS='tables-filter tinymce' riscv64/adminer
```
...
```console
$ docker run --link some_database:db -p 8080:8080 -e ADMINER_PLUGINS='login-servers' riscv64/adminer
...
```console
$ docker run --link some_database:db -p 8080:8080 -e ADMINER_DESIGN='nette' riscv64/adminer
```
...
```console
docker run -p 8080:8080 -e ADMINER_DEFAULT_SERVER=mysql riscv64/adminer
```
