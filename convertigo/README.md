...
**Note:** this is the "per-architecture" repository for the `arm32v6` builds of [the `convertigo` official image](https://hub.docker.com/_/convertigo) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
```console
$ docker run -d --name C8O --link fullsync:couchdb -p 28080:28080 arm32v6/convertigo
```
...
```console
$ docker run -d --name C8O -e JAVA_OPTS="-Dconvertigo.engine.fullsync.pouchdb=true" -p 28080:28080 arm32v6/convertigo
```
...
```console
$ docker run --name C8O -v $(pwd):/workspace -d -p 28080:28080 arm32v6/convertigo
```
...
```console
$ docker run -d --name C8O -e CONVERTIGO_ADMIN_USER=administrator -e CONVERTIGO_ADMIN_PASSWORD=s3cret -p 28080:28080 arm32v6/convertigo
```
...
```console
$ docker run -d --name C8O -e CONVERTIGO_TESTPLATFORM_USER=tp_user -e CONVERTIGO_TESTPLATFORM_PASSWORD=s3cret -p 28080:28080 arm32v6/convertigo
```
...
```console
$ docker run -d --name C8O -v <my SSL folder>:/ssl -p 28443:28443 arm32v6/convertigo
```

If you want to expose both **HTTP** and **HTTPS** you can expose both **ports**:

```console
$ docker run -d --name C8O -v <my SSL folder>:/ssl -p 28080:28080 -p 28443:28443 arm32v6/convertigo
```
...
$ docker run -d --name C8O -e SSL_KEY_B64="$SSL_KEY_B64" -e SSL_CERT_B64="$SSL_CERT_B64" -e SSL_CHAIN_B64="$SSL_CHAIN_B64" -p 28443:28443 arm32v6/convertigo
```
...
```console
$ docker run -d --name C8O -e SSL_SELFSIGNED=mycomputer -p 28443:28443 arm32v6/convertigo
```

Generated files can be retrieved if the `/ssl` mount point is configured on folder without `cert.pem` nor `key.pem`.

```console
$ docker run -d --name C8O -v <my empty SSL folder>:/ssl -e SSL_SELFSIGNED=mycomputer -p 28443:28443 arm32v6/convertigo
```
...
```console
$ docker run -d --name C8O -e JAVA_OPTS="-DjvmRoute=server1" -p 28080:28080 arm32v6/convertigo
```
...
```console
$ docker run -d --name C8O -e JXMX="4096" -p 28080:28080 arm32v6/convertigo
```
...
```console
$ docker run -d --name C8O -e COOKIE_PATH="/convertigo" -p 28080:28080 arm32v6/convertigo
```
...
```console
$ docker run -d --name C8O -e COOKIE_SECURE="true" -p 28080:28080 arm32v6/convertigo
```
...
```console
$ docker run -d --name C8O -e COOKIE_SAMESITE=lax -p 28080:28080 arm32v6/convertigo
```
...
```console
$ docker run -d --name C8O -e SESSION_TIMEOUT=5 -p 28080:28080 arm32v6/convertigo
```
...
```console
$ docker run -d --name C8O -e DISABLE_SUDO=true -p 28080:28080 arm32v6/convertigo
```
