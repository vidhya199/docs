...
```console
$ docker run --rm s390x/influxdb:2.0 influxd print-config > config.yml
```
...
```console
$ docker run -p 8086:8086 \
      s390x/influxdb:2.0 --storage-wal-fsync-delay=15m
```
...
$ docker run -p 8086:8086 \
      ...
      s390x/influxdb:1.8 -config /root/influxdb/influxdb.conf
```
...
```console
$ docker run --rm s390x/influxdb:1.8 influxd config > influxdb.conf
```
...
$ docker run -p 8086:8086 \
      ...
      s390x/influxdb:1.8 -config /etc/influxdb/influxdb.conf
```
...
```console
$ docker run --name=influxdb -d -p 8086:8086 s390x/influxdb:1.8
```
...
```console
$ docker run --rm --link=influxdb -it s390x/influxdb:1.8 influx -host influxdb
```
...
$ docker run --rm \
      ...
      -v $PWD:/var/lib/influxdb \
      s390x/influxdb:1.8 /init-influxdb.sh
```
