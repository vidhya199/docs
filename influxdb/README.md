...
[![mips64le/influxdb build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/mips64le/job/influxdb.svg?label=mips64le/influxdb%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/mips64le/job/influxdb/)
...
```console
$ docker run --rm mips64le/influxdb:2.0 influxd print-config > config.yml
```
...
```console
$ docker run -p 8086:8086 \
      mips64le/influxdb:2.0 --storage-wal-fsync-delay=15m
```
...
$ docker run -p 8086:8086 \
      ...
      mips64le/influxdb:1.8 -config /root/influxdb/influxdb.conf
```
...
```console
$ docker run --rm mips64le/influxdb:1.8 influxd config > influxdb.conf
```
...
$ docker run -p 8086:8086 \
      ...
      mips64le/influxdb:1.8 -config /etc/influxdb/influxdb.conf
```
...
```console
$ docker run --name=influxdb -d -p 8086:8086 mips64le/influxdb:1.8
```
...
```console
$ docker run --rm --link=influxdb -it mips64le/influxdb:1.8 influx -host influxdb
```
...
$ docker run --rm \
      ...
      -v $PWD:/var/lib/influxdb \
      mips64le/influxdb:1.8 /init-influxdb.sh
```
