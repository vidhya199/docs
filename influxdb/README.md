...
**Note:** this is the "per-architecture" repository for the `arm32v6` builds of [the `influxdb` official image](https://hub.docker.com/_/influxdb) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
```console
$ docker run --rm arm32v6/influxdb:2.0 influxd print-config > config.yml
```
...
```console
$ docker run -p 8086:8086 \
      arm32v6/influxdb:2.0 --storage-wal-fsync-delay=15m
```
...
$ docker run -p 8086:8086 \
      ...
      arm32v6/influxdb:1.8 -config /root/influxdb/influxdb.conf
```
...
```console
$ docker run --rm arm32v6/influxdb:1.8 influxd config > influxdb.conf
```
...
$ docker run -p 8086:8086 \
      ...
      arm32v6/influxdb:1.8 -config /etc/influxdb/influxdb.conf
```
...
```console
$ docker run --name=influxdb -d -p 8086:8086 arm32v6/influxdb:1.8
```
...
```console
$ docker run --rm --link=influxdb -it arm32v6/influxdb:1.8 influx -host influxdb
```
...
$ docker run --rm \
      ...
      -v $PWD:/var/lib/influxdb \
      arm32v6/influxdb:1.8 /init-influxdb.sh
```
