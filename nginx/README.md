...
To run arm32v6/nginx in read-only mode, you will need to mount a Docker volume to every location where arm32v6/nginx writes information. The default arm32v6/nginx configuration requires write access to `/var/cache/nginx` and `/var/run`. This can be easily accomplished by running arm32v6/nginx as follows:
...
## `arm32v6/nginx:<version>-perl` / `arm32v6/nginx:<version>-alpine-perl`

Starting with arm32v6/nginx:1.13.0 / mainline and arm32v6/nginx:1.12.0 / stable, the perl module has been removed from the default images. A separate `-perl` tag variant is available if you wish to use the perl module.

## `arm32v6/nginx:<version>-slim`

This image does not contain the common packages contained in the default tag and only contains the minimal packages needed to run `arm32v6/nginx`. Unless you are working in an environment where *only* the `arm32v6/nginx` image will be deployed and you have space constraints, we highly recommend using the default image of this repository.
