...
```sh
docker run --name phpmyadmin -d --link mysql_db_server:db -p 8080:80 arm32v6/phpmyadmin
```
...
```sh
docker run --name phpmyadmin -d -e PMA_HOST=dbhost -p 8080:80 arm32v6/phpmyadmin
```
...
```sh
docker run --name phpmyadmin -d -e PMA_ARBITRARY=1 -p 8080:80 arm32v6/phpmyadmin
```
...
```sh
docker run --name phpmyadmin -d --link mysql_db_server:db -p 8080:80 -v /some/local/directory/config.user.inc.php:/etc/phpmyadmin/config.user.inc.php arm32v6/phpmyadmin
```
...
```sh
docker run --name phpmyadmin -d -e PMA_PASSWORD_FILE=/run/secrets/db_password.txt -p 8080:80 arm32v6/phpmyadmin
```
...
The `arm32v6/phpmyadmin` images come in many flavors, each designed for a specific use case.

## `arm32v6/phpmyadmin:<version>`

This is the defacto image. If you are unsure about what your needs are, you probably want to use this one. It is designed to be used both as a throw away container (mount your source code and start the container to start your app), as well as the base to build other images off of.

## `arm32v6/phpmyadmin:<version>-alpine`
