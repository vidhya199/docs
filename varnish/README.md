...
$ docker run --tmpfs /var/lib/varnish/varnishd:exec -p 8080:7777 -e VARNISH_HTTP_PORT=7777 arm64v8/varnish
```

### Extra arguments

Additionally, you can add arguments to `docker run` after `arm64v8/varnish`, if the first argument starts with a `-`, the whole list will be appendend to the [default command](https://github.com/varnish/docker-varnish/blob/master/fresh/debian/scripts/docker-varnish-entrypoint):
...
```dockerfile
FROM arm64v8/varnish:7.1-alpine
