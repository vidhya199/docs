...
Additionally, you can add arguments to `docker run` after `ppc64le/varnish`, if the first argument starts with a `-`, the whole list will be appendend to the [default command](https://github.com/varnish/docker-varnish/blob/master/fresh/debian/scripts/docker-varnish-entrypoint):
...
```dockerfile
FROM ppc64le/varnish:7.1-alpine
